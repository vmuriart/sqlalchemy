...
attributes = Table('attributes', meta,
                   Column('element_id', Integer,
                          ForeignKey('elements.element_id'), primary_key=True),
...
mapper(_Node, elements, properties={
    ...
    'attributes': relationship(_Attribute, lazy='joined',
...
class ElementTreeMarshal(object):
    ...
    def __set__(self, document, element):
        def traverse(node):
            ...
            n.attributes = [_Attribute(str(k), str(v)) for k, v in
                            node.attrib.items()]
            return n
...
d = session.query(Document).join('_root', aliased=True).filter(
    _Node.tag == 'somefile'). \
...
def find_document(path, compareto):
    ...
    for i, match in enumerate(
        ...
        query = query.join(attribute, aliased=True,
                           from_joinpoint=True).filter(_Node.tag == token)
        attribute = 'children'
        if attrname:
            if attrvalue:
                query = query.join('attributes', aliased=True,
                                   from_joinpoint=True).filter(
                    and_(_Attribute.name == attrname,
                         _Attribute.value == attrvalue))
            else:
                query = query.join('attributes', aliased=True,
                                   from_joinpoint=True).filter(
                    _Attribute.name == attrname)
    return query.options(lazyload('_root')).filter(
        _Node.text == compareto).all()
