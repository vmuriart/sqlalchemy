...
class ReflectionTest(fixtures.TestBase, ComparesTables):
    ...
    def test_reserved(self):
        ...
        Table('false', meta,
              ...
              sa.CheckConstraint('{0!s} <> 1'.format(quoter(check_col)),
                                 name='limit')
              )
...
class UnicodeReflectionTest(fixtures.TestBase):
    ...
    def test_has_table(self):
        for tname, cname, ixname in self.names:
            assert testing.db.has_table(
                tname), "Can't detect name {0!s}".format(tname)
...
def createTables(meta, schema=None):
    ...
    users = Table('users', meta,
                  ...
                  Column('parent_user_id', sa.Integer,
                         sa.ForeignKey(
                             '{0!s}users.user_id'.format(schema_prefix))),
                  Column('test6', sa.Date, nullable=False),
                  Column('test7', sa.Text),
                  Column('test8', sa.LargeBinary),
                  Column('test_passivedefault2', sa.Integer,
    ...
    dingalings = Table("dingalings", meta,
                       ...
                       Column('address_id', sa.Integer,
                              sa.ForeignKey(
                                  '{0!s}email_addresses.address_id'.format(
                                      schema_prefix))),
...
def _create_views(con, schema=None):
    for table_name in ('users', 'email_addresses'):
        ...
        query = "CREATE VIEW {0!s} AS SELECT * FROM {1!s}".format(view_name,
                                                                  fullname)
        con.execute(sa.sql.text(query))
