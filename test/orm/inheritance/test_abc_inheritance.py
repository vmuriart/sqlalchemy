...
def produce_test(parent, child, direction):
    ...
    class ABCTest(fixtures.MappedTest):
        ...
        def define_tables(cls, metadata):
            ...
            if "a" == parent and direction == MANYTOONE:
                ta.append(Column('child_id', Integer,
                                 ForeignKey("{0!s}.id".format(child),
                                            use_alter=True, name="foo")))
            elif "a" == child and direction == ONETOMANY:
                ta.append(Column('parent_id', Integer,
                                 ForeignKey("{0!s}.id".format(parent),
                                            use_alter=True, name="foo")))
            ta = Table(*ta)
            ...
            if "b" == parent and direction == MANYTOONE:
                tb.append(Column('child_id', Integer,
                                 ForeignKey("{0!s}.id".format(child),
                                            use_alter=True, name="foo")))
            elif "b" == child and direction == ONETOMANY:
                tb.append(Column('parent_id', Integer,
                                 ForeignKey("{0!s}.id".format(parent),
                                            use_alter=True, name="foo")))
            tb = Table(*tb)
            ...
            if "c" == parent and direction == MANYTOONE:
                tc.append(Column('child_id', Integer,
                                 ForeignKey("{0!s}.id".format(child),
                                            use_alter=True, name="foo")))
            elif "c" == child and direction == ONETOMANY:
                tc.append(Column('parent_id', Integer,
                                 ForeignKey("{0!s}.id".format(parent),
                                            use_alter=True, name="foo")))
            tc = Table(*tc)
        ...
        def test_roundtrip(self):
            ...
            abcjoin = polymorphic_union(
                {"a": ta.select(tb.c.id == None,
                 ...
                 "b": ta.join(tb, onclause=atob).outerjoin(tc,
                                                           onclause=btoc).select(
                     tc.c.id == None).reduce_columns(),
            ...
            mapper(A, ta, polymorphic_on=abcjoin.c.type,
                   with_polymorphic=('*', abcjoin), polymorphic_identity="a")
            mapper(B, tb, polymorphic_on=bcjoin.c.type,
                   with_polymorphic=('*', bcjoin), polymorphic_identity="b",
                   inherits=A, inherit_condition=atob)
            mapper(C, tc, polymorphic_identity="c", inherits=B,
    ...
    ABCTest.__name__ = "Test{0!s}To{1!s}{2!s}".format(parent, child, (
