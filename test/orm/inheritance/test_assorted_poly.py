...
class RelationshipTest4(fixtures.MappedTest):
    ...
    def test_many_to_one_polymorphic(self):
        ...
        class Engineer(Person):
            def __repr__(self):
                return "Engineer {0!s}, status {1!s}".format(self.name,
                                                             self.status)

        class Manager(Person):
            def __repr__(self):
                return "Manager {0!s}, status {1!s}".format(self.name,
                                                            self.longer_status)
...
class RelationshipTest7(fixtures.MappedTest):
    ...
    def test_manytoone_lazyload(self):
        ...
        class Car(PersistentObject):
            def __repr__(self):
                return "Car number {0:d}, name {1!s}".format(self.car_id,
                                                             self.name)

        class Offraod_Car(Car):
            def __repr__(self):
                return "Offroad Car number {0:d}, name {1!s}".format(
...
class GenerativeTest(fixtures.TestBase, AssertsExecutionResults):
    ...
    def test_join_to(self):
        ...
        for i in range(1, 5):
            ...
            session.add(
                Engineer(name="E{0:d}".format(i), field="X", status=st))
...
class CustomPKTest(fixtures.MappedTest):
    ...
    def define_tables(cls, metadata):
        ...
        t2 = Table('t2', metadata,
                   Column('t2id', Integer, ForeignKey('t1.id'),
                          primary_key=True),
