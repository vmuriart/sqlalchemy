...
class Location(BaseObject):
    def __repr__(self):
        return "{0!s}({1!s}, {2!s})".format(self.__class__.__name__, str(
            getattr(self, 'issue_id', None)), repr(str(self._name.name)))
    ...
    def _set_name(self, name):
        ...
        s = session.query(LocationName).filter(
            LocationName.name == name).first()
        session.expunge_all()
        if s is not None:
            self._name = s
...
class PageSize(BaseObject):
    def __repr__(self):
        return "{0!s}({1!s}x{2!s}, {3!s})".format(self.__class__.__name__,
...
def _generate_round_trip_test(use_unions=False, use_joins=False):
    def test_roundtrip(self):
        ...
        issue_mapper = mapper(Issue, issue_table, properties={
            'publication': relationship(Publication, backref=backref('issues',
        ...
        location_mapper = mapper(Location, location_table, properties={
            'issue': relationship(Issue,
                                  backref=backref('locations', lazy='joined',
                                                  cascade="all, delete-orphan")),
    ...
    test_roundtrip = function_named(
        test_roundtrip, "test_{0!s}".format((not use_union and (
            use_joins and "joins" or "select") or "unions")))
