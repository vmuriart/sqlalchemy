...
class InheritTest(fixtures.MappedTest):
    ...
    def define_tables(cls, metadata):
        ...
        specification_table = Table('specification', metadata,
                                    Column('spec_line_id', Integer,
                                           primary_key=True,
                                           test_needs_autoincrement=True),
                                    Column('master_id', Integer,
                                           ForeignKey("products.product_id"),
                                           nullable=True),
                                    Column('slave_id', Integer,
                                           ForeignKey("products.product_id"),
                                           nullable=True),
        ...
        class Assembly(Product):
            def __repr__(self):
                return Product.__repr__(self) + " " + " ".join(
                    [x + "=" + repr(getattr(self, x, None)) for x in
        ...
        specification_mapper = mapper(SpecLine, specification_table,
                                      properties=dict(
                                          master=relationship(Assembly,
                                                              foreign_keys=[
                                                                  specification_table.c.master_id],
                                          ...
                                          slave=relationship(Product,
                                                             foreign_keys=[
                                                                 specification_table.c.slave_id],
    ...
    def test_three(self):
        ...
        raster_document_mapper = mapper(RasterDocument,
