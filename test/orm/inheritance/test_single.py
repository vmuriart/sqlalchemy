...
class SingleInheritanceTest(testing.AssertsCompiledSQL, fixtures.MappedTest):
    ...
    def test_select_from(self):
        ...
        eq_(
            sess.query(Manager).select_from(
                employees.select().limit(10)).all(),
            [m1, m2]
        )
    ...
    def test_type_filtering(self):
        ...
        assert len(
            rq.filter(Report.employee.of_type(Manager).has()).all()) == 1
        assert len(
            rq.filter(Report.employee.of_type(Engineer).has()).all()) == 0
...
class RelationshipFromSingleTest(testing.AssertsCompiledSQL,
                                 fixtures.MappedTest):
    @classmethod
    def define_tables(cls, metadata):
    ...
    def test_subquery_load(self):
        ...
        subq = context.attributes[('subquery',
                                   (class_mapper(Manager),
                                    class_mapper(Manager).attrs.stuff))]
...
class RelationshipToSingleTest(testing.AssertsCompiledSQL,
                               fixtures.MappedTest):
    __dialect__ = 'default'
    ...
    def test_of_type(self):
        ...
        sess.expunge_all()
        eq_(
            sess.query(Company).filter(
                Company.employees.of_type(JuniorEngineer).any()).all(),
            [
                Company(name='c1'),
            ]
        )
        ...
        eq_(
            sess.query(Company).join(
                Company.employees.of_type(JuniorEngineer)).all(),
            [
                Company(name='c1'),
            ]
        )
    ...
    def test_outer_join_prop_alias(self):
        ...
        sess = create_session()
        self.assert_compile(
            sess.query(Company, eng_alias.name).outerjoin(eng_alias,
                                                          Company.engineers),
            "SELECT companies.company_id AS companies_company_id, "
    ...
    def test_relationship_to_subclass(self):
        ...
        sess.expunge_all()
        eq_(sess.query(Company).options(joinedload('engineers')).order_by(
            Company.name).all(),
            [
                Company(name='c1', engineers=[JuniorEngineer(name='Ed')]),
                Company(name='c2', engineers=[Engineer(name='Kurt')])
        ...
        sess.expunge_all()
        eq_(sess.query(Company, Employee).join(Company.engineers).order_by(
            Company.name).all(),
            [
        ...
        sess.expunge_all()
        eq_(sess.query(Company, Engineer).join(Company.engineers).order_by(
            Company.name).all(),
            [
                (Company(name='c1'), JuniorEngineer(name='Ed')),
                (Company(name='c2'), Engineer(name='Kurt'))
        ...
        def go():
            ...
            eq_(sess.query(Company). \
                filter(Company.company_id == Engineer.company_id).filter(
                Engineer.name.in_(['Tom', 'Kurt'])).all(),
                [
                    Company(name='c2')
                ]
