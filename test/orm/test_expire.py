...
class ExpireTest(_fixtures.FixtureTest):
    ...
    def test_expire_committed(self):
        ...
        assert o.isopen == 1
        assert attributes.instance_state(o).dict[
                   'description'] == 'order 3 modified'
    ...
    def test_state_noload_to_lazy(self):
        ...
        # expire, it stays
        sess.expire(u1)
        assert 'addresses' not in attributes.instance_state(
            u1).expired_attributes
        assert isinstance(
            attributes.instance_state(u1).callables['addresses'],
            strategies.LoadLazyAttribute
        )
...
class ExpiredPendingTest(_fixtures.FixtureTest):
    ...
    def test_expired_pending(self):
        ...
        # insert a new row
        sess.execute(addresses.insert(),
                     dict(email_address='a3', user_id=u1.id))
