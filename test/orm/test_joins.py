...
class InheritedJoinTest(fixtures.MappedTest, AssertsCompiledSQL):
    ...
    def test_prop_with_polymorphic_1(self):
        ...
        self.assert_compile(
            sess.query(Person).with_polymorphic(Manager).
                ...
                join('paperwork').filter(
                Paperwork.description.like('%review%')),
    ...
    def test_explicit_polymorphic_join_one(self):
        ...
        self.assert_compile(
            sess.query(Company).join(Engineer).filter(
                Engineer.engineer_name == 'vlad'),
    ...
    def test_explicit_polymorphic_join_two(self):
        ...
        self.assert_compile(
            sess.query(Company).join(Engineer,
                                     Company.company_id == Engineer.company_id).
    ...
    def test_multiple_adaption(self):
        ...
        self.assert_compile(
            sess.query(Company).join(people.join(engineers),
                                     Company.employees).
        ...
        self.assert_compile(
            sess.query(Company).join(people.join(engineers),
                ...
                filter(Engineer.name == 'dilbert').filter(
                Machine.name == 'foo'),
            "SELECT companies.company_id AS companies_company_id, companies.name AS "
            "companies_name "
            "FROM companies JOIN (people "
...
class JoinTest(QueryTest, AssertsCompiledSQL):
    ...
    def test_overlapping_paths(self):
        ...
        for aliased in (True, False):
            ...
            result = create_session().query(User).join('orders', 'items',
                                                       aliased=aliased). \
                filter_by(id=3).join('orders', 'address',
                                     aliased=aliased).filter_by(id=1).all()
    ...
    def test_overlapping_paths_outerjoin(self):
        ...
        result = create_session().query(User).outerjoin('orders', 'items'). \
            filter_by(id=3).outerjoin('orders', 'address').filter_by(
            id=1).all()
        assert [User(id=7, name='jack')] == result
    ...
    def test_from_joinpoint(self):
        ...
        for oalias, ialias in [(True, True), (False, False), (True, False),
                               (False, True)]:
            eq_(
    ...
    def test_backwards_join(self):
        ...
        eq_(
            sess.query(User, Address).join(Address.user).filter(
                Address.email_address == 'ed@wood.com').all(),
    ...
    def test_replace_multiple_from_clause(self):
        ...
        self.assert_compile(
            sess.query(Address, User).join(Address.dingaling).join(User.orders,
                                                                   Order.items),
    ...
    def test_multiple_adaption(self):
        ...
        self.assert_compile(
            sess.query(User).join(User.orders, Order.items,
                                  aliased=True).filter(Order.id == 7).filter(
                Item.id == 8),
    ...
    def test_orderby_arg_bug(self):
        ...
        sess = create_session()
        # no arg error
        result = sess.query(User).join('orders', aliased=True).order_by(
            Order.id).reset_joinpoint().order_by(users.c.id).all()
    ...
    def test_no_onclause(self):
        ...
        eq_(
            sess.query(User).select_from(
                join(User, Order).join(Item, Order.items)).filter(
                Item.description == 'item 4').all(),
            [User(name='jack')]
        )

        eq_(
            sess.query(User.name).select_from(
                join(User, Order).join(Item, Order.items)).filter(
                Item.description == 'item 4').all(),
            [('jack',)]
        )
    ...
    def test_aliased_classes(self):
        ...
        l = q.select_from(outerjoin(User, AdAlias)).filter(
            AdAlias.email_address == 'ed@bettyboop.com').all()
        eq_(l, [(user8, address3)])

        l = q.select_from(outerjoin(User, AdAlias, 'addresses')).filter(
            AdAlias.email_address == 'ed@bettyboop.com').all()
        eq_(l, [(user8, address3)])
        ...
        l = q.join(AdAlias.user).filter(User.name == 'ed').order_by(User.id,
                                                                    AdAlias.id)
        eq_(l.all(),
    ...
    def test_implicit_joins_from_aliases(self):
        ...
        eq_(
            sess.query(OrderAlias).join('items').filter_by(
                description='item 3'). \
        ...
        eq_(
            ...
            [
                (User(name='jack', id=7),
                 Order(address_id=1, description='order 1', isopen=0,
                       user_id=7, id=1), 'item 3'),
                (User(name='jack', id=7),
                 Order(address_id=1, description='order 3', isopen=1,
                       user_id=7, id=3), 'item 3'),
                (User(name='fred', id=9),
                 Order(address_id=4, description='order 2', isopen=0,
                       user_id=9, id=2), 'item 3')
            ]
        )
    ...
    def test_aliased_classes_m2m(self):
        ...
        q = sess.query(Order, IAlias).select_from(
            join(Order, IAlias, 'items')).filter(
            IAlias.description == 'item 3')
        l = q.all()
        eq_(l,
            [
                (order1, item3),
    ...
    def test_reset_joinpoint(self):
        ...
        for aliased in (True, False):
            # load a user who has an order that contains item id 3 and address id 1 (order 3, owned by jack)
            result = create_session().query(User).join('orders', 'items',
                                                       aliased=aliased).filter_by(
                id=3).reset_joinpoint().join('orders', 'address',
                                             aliased=aliased).filter_by(
                id=1).all()
            assert [User(id=7, name='jack')] == result
            ...
            result = create_session().query(User).outerjoin('orders', 'items',
                                                            aliased=aliased).filter_by(
                id=3).reset_joinpoint().outerjoin('orders', 'address',
                                                  aliased=aliased).filter_by(
                id=1).all()
            assert [User(id=7, name='jack')] == result
    ...
    def test_overlap_with_aliases(self):
        ...
        result = create_session().query(User).select_from(
            users.join(oalias)).filter(
            oalias.c.description.in_(["order 1", "order 2", "order 3"])).join(
            'orders', 'items').order_by(User.id).all()
        assert [User(id=7, name='jack'), User(id=9, name='fred')] == result

        result = create_session().query(User).select_from(
            users.join(oalias)).filter(
            oalias.c.description.in_(["order 1", "order 2", "order 3"])).join(
            'orders', 'items').filter_by(id=4).all()
        assert [User(id=7, name='jack')] == result
    ...
    def test_aliased(self):
        ...
        q = sess.query(User).join('addresses', aliased=True).filter(
            or_(Address.email_address == 'jack@bean.com',
        ...
        q = sess.query(User).join('orders', aliased=True).filter(
            Order.description == "order 3").join('orders', 'items',
                                                 aliased=True).filter(
        ...
        q = sess.query(User).join('orders').filter(
            Order.description == "order 3").join('orders', 'items').filter(
            Item.description == "item 1")
        assert [] == q.all()
        assert q.count() == 0
        ...
        q = sess.query(User).join('orders', aliased=True).filter(
            Order.description == "order 3").join('orders',
                                                 aliased=True).filter(
    ...
    def test_aliased_order_by(self):
        ...
        ualias = aliased(User)
        eq_(
            sess.query(User, ualias).filter(User.id > ualias.id).order_by(
                desc(ualias.id), User.name).all(),
            [
    ...
    def test_plain_table(self):
        ...
        eq_(
            sess.query(User.name).join(addresses,
                                       User.id == addresses.c.user_id).order_by(
                User.id).all(),
            [('jack',), ('ed',), ('ed',), ('ed',), ('fred',)]
        )
    ...
    def test_select_from(self):
        ...
        sess = create_session()
        self.assert_compile(
            sess.query(Item.id).select_from(User).join(User.orders).join(
                Order.items),
        ...
        self.assert_compile(
            sess.query(Item.id).select_from(User).join(Item,
                                                       User.id == Item.id),
    ...
    def test_from_self_resets_joinpaths(self):
        ...
        self.assert_compile(
            sess.query(Item).join(Item.keywords).from_self(Keyword).join(
                Item.keywords),
...
class JoinFromSelectableTest(fixtures.MappedTest, AssertsCompiledSQL):
    ...
    def test_select_mapped_to_mapped_explicit_left(self):
        ...
        self.assert_compile(
            sess.query(subq.c.count, T1.id).select_from(subq).join(T1,
                                                                   subq.c.t1_id == T1.id),
    ...
    def test_select_mapped_to_select_explicit_left(self):
        ...
        self.assert_compile(
            sess.query(subq.c.count, T1.id).select_from(T1).join(subq,
                                                                 subq.c.t1_id == T1.id),
            "SELECT anon_1.count AS anon_1_count, table1.id AS table1_id "
    ...
    def test_mapped_select_to_mapped_explicit_left(self):
        ...
        self.assert_compile(
            sess.query(T1.id, subq.c.count).select_from(subq).join(T1,
                                                                   subq.c.t1_id == T1.id),
            "SELECT table1.id AS table1_id, anon_1.count AS anon_1_count "
            "FROM (SELECT table2.t1_id AS t1_id, count(table2.id) AS count "
    ...
    def test_mapped_select_to_select_explicit_left(self):
        ...
        self.assert_compile(
            sess.query(T1.id, subq.c.count).select_from(T1).join(subq,
                                                                 subq.c.t1_id == T1.id),
            "SELECT table1.id AS table1_id, anon_1.count AS anon_1_count "
...
class MultiplePathTest(fixtures.MappedTest, AssertsCompiledSQL):
    ...
    def test_basic(self):
        ...
        q = create_session().query(T1).join('t2s_1').filter(
            t2.c.id == 5).reset_joinpoint().join('t2s_2')
        self.assert_compile(
            q,
...
class JoinToNonPolyAliasesTest(fixtures.MappedTest, AssertsCompiledSQL):
    ...
    def test_join_parent_child(self):
        ...
        sess = Session()
        self.assert_compile(
            sess.query(Parent).join(Parent.npc).filter(
...
class SelfReferentialTest(fixtures.MappedTest, AssertsCompiledSQL):
    ...
    def test_join_1(self):
        ...
        node = sess.query(Node).join('children', aliased=True).filter_by(
            data='n122').first()
    ...
    def test_join_2(self):
        Node = self.classes.Node
        sess = create_session()
        ret = sess.query(Node.data).join(Node.children,
    ...
    def test_join_3(self):
        Node = self.classes.Node
        sess = create_session()
        node = sess.query(Node).join('children', 'children',
                                     aliased=True).filter_by(
            data='n122').first()
        assert node.data == 'n1'
    ...
    def test_join_4(self):
        Node = self.classes.Node
        sess = create_session()
        node = sess.query(Node).filter_by(data='n122').join('parent',
                                                            aliased=True).filter_by(
            data='n12'). \
            join('parent', aliased=True, from_joinpoint=True).filter_by(
            data='n1').first()
    ...
    def test_from_self_inside_excludes_outside(self):
        ...
        # on the outside
        self.assert_compile(
            sess.query(Node).filter(Node.data == 'n122').from_self(n1,
                                                                   Node.id),
    ...
    def test_explicit_join_9(self):
        ...
        node = sess.query(Node).select_from(
            join(Node, n1, 'parent').join(n2, 'parent')). \
            filter(and_(Node.data == 'n122', n1.data == 'n12',
                        n2.data == 'n1')).first()
        assert node.data == 'n122'
    ...
    def test_explicit_join_10(self):
        ...
        eq_(
            list(sess.query(Node).select_from(
                join(Node, n1, 'parent').join(n2, 'parent')). \
    ...
    def test_join_to_nonaliased(self):
        ...
        # explicit (new syntax)
        eq_(
            sess.query(n1).join(Node, n1.parent).filter(
                Node.data == 'n1').all(),
            [Node(parent_id=1, data='n11', id=2),
             Node(parent_id=1, data='n12', id=3),
             Node(parent_id=1, data='n13', id=4)]
    ...
    def test_any(self):
        ...
        eq_(sess.query(Node).filter(
            Node.children.any(Node.data == 'n1')).all(), [])
        eq_(sess.query(Node).filter(
            Node.children.any(Node.data == 'n12')).all(), [Node(data='n1')])
        eq_(sess.query(Node).filter(~Node.children.any()).order_by(
            Node.id).all(),
            [Node(data='n11'), Node(data='n13'), Node(data='n121'),
    ...
    def test_has(self):
        ...
        eq_(sess.query(Node).filter(
            Node.parent.has(Node.data == 'n12')).order_by(Node.id).all(),
            [Node(data='n121'), Node(data='n122'), Node(data='n123')])
        eq_(sess.query(Node).filter(
            Node.parent.has(Node.data == 'n122')).all(), [])
        eq_(sess.query(Node).filter(~Node.parent.has()).all(),
            [Node(data='n1')])
...
class SelfReferentialM2MTest(fixtures.MappedTest):
    ...
    def insert_data(cls):
        ...
        mapper(Node, nodes, properties={
            'children': relationship(Node, lazy='select',
    ...
    def test_any(self):
        ...
        eq_(sess.query(Node).filter(Node.children.any(Node.data == 'n3'
            ...
            Node.data).all(), [Node(data='n1'), Node(data='n2')])
    ...
    def test_contains(self):
        ...
        eq_(sess.query(Node).filter(Node.children.contains(n4)).order_by(
            Node.data).all(),
            [Node(data='n1'), Node(data='n3')])
        eq_(sess.query(Node).filter(not_(Node.children.contains(n4))).order_by(
            Node.data).all(),
